...
def update_scatter(site_dropdown, payload_range):
    return build_scatter(site_dropdown, payload_range)
...
def build_scatter(site_dropdown,slider_range):
    ...
    if site_dropdown == 'ALL':
        fig = px.scatter(spacex_df[mask], x="Payload Mass (kg)", y="class", color="Booster Version Category", title='Success Rate for All Sites')
   
    else:
        filtered_df = spacex_df[(mask) & (spacex_df['Launch Site'] == site_dropdown)]
        fig = px.scatter(filtered_df, x="Payload Mass (kg)", y="class", color="Booster Version Category", title=f"Payload and Booster Version Success Rates for {site_dropdown}")
